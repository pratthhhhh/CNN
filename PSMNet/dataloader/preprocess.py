...
def scale_random_crop(input_size, scale_size=None, normalize=__imagenet_stats):
    ...
    if scale_size != input_size:
        # transforms.Scale was removed; use transforms.Resize
...
def inception_preproccess(input_size, normalize=__imagenet_stats):
    return transforms.Compose([
        transforms.RandomResizedCrop(input_size),   # replaces removed RandomSizedCrop
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        transforms.Normalize(**normalize)
    ])
...
class Grayscale(object):
    ...
    def __call__(self, img):
        gs = img.clone()
        # multi-arg add_ was deprecated; split into separate operations
        gs[0].mul_(0.299).add_(gs[1].mul(0.587)).add_(gs[2].mul(0.114))
        gs[1].copy_(gs[0])
        gs[2].copy_(gs[0])
        return gs
