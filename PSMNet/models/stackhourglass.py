...
class hourglass(nn.Module):
    def __init__(self, inplanes):
        ...
        self.conv5 = nn.Sequential(
            nn.ConvTranspose3d(inplanes * 2, inplanes * 2, kernel_size=3, padding=1,
                               output_padding=1, stride=2, bias=False),
            nn.BatchNorm3d(inplanes * 2))  # + conv2

        self.conv6 = nn.Sequential(
            nn.ConvTranspose3d(inplanes * 2, inplanes, kernel_size=3, padding=1,
                               output_padding=1, stride=2, bias=False),
...
class PSMNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        # Build cost volume — device-agnostic, no Variable wrapper needed
        cost = torch.zeros(
            refimg_fea.size()[0],
            refimg_fea.size()[1] * 2,
            self.maxdisp // 4,
