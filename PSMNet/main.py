...
def validate(imgL, imgR, disp_true):
    ...
    # 384 × 768 is already divisible by 16, so no padding needed.
    # Guard added for safety in case images differ at inference time.
    if imgL.shape[2] % 16 != 0:
        top_pad = (imgL.shape[2] // 16 + 1) * 16 - imgL.shape[2]
        imgL = F.pad(imgL, (0, 0, top_pad, 0))
        imgR = F.pad(imgR, (0, 0, top_pad, 0))
    else:
        top_pad = 0

    if imgL.shape[3] % 16 != 0:
        right_pad = (imgL.shape[3] // 16 + 1) * 16 - imgL.shape[3]
        imgL = F.pad(imgL, (0, right_pad, 0, 0))
        imgR = F.pad(imgR, (0, right_pad, 0, 0))
    else:
        right_pad = 0
