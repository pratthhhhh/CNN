...
all_left_img, all_right_img, all_left_disp, \
    test_left_img, test_right_img, test_left_disp = ls.dataloader(args.datapath)

TrainImgLoader = torch.utils.data.DataLoader(
    DA.myImageFloder(all_left_img, all_right_img, all_left_disp, True),
    batch_size=12, shuffle=True, num_workers=8, drop_last=False, pin_memory=True)

TestImgLoader = torch.utils.data.DataLoader(
    DA.myImageFloder(test_left_img, test_right_img, test_left_disp, False),
    batch_size=8, shuffle=False, num_workers=4, drop_last=False, pin_memory=True)
...
if args.loadmodel is not None:
    state_dict = torch.load(args.loadmodel, weights_only=False)
...
def test(imgL, imgR, disp_true):
    ...
    # Convert numpy arrays coming from the dataloader if needed
    if not isinstance(imgL, torch.Tensor):
        imgL = torch.FloatTensor(imgL)
    if not isinstance(imgR, torch.Tensor):
